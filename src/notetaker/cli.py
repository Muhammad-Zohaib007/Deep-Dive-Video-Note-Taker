...
def process(
    ...
    console.print(Panel(
        ...
        f"[dim]Whisper: {whisper_model.value} | LLM: {ollama_model} | Format: {output.value}[/dim]"
...
def batch(
    sources: list[str] = typer.Argument(
        None, help="URLs or file paths. Or use --file to read from a file.",
    ),
    ...
    for i, r in enumerate(results, 1):
        ...
        table.add_row(str(i), r["source"][:40], r["video_id"], f"[{status_style}]{r['status']}[/{status_style}]", r["time"])
        if r["status"] == "success":
            succeeded += 1
...
def _export_output(generated_output, metadata, output: OutputFormat, config) -> None:
    ...
    if output == OutputFormat.MARKDOWN:
        ...
        if token:
            ...
            try:
                page_url = asyncio.run(export_to_notion(
                    generated_output, metadata, token=token,
